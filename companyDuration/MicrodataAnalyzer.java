...
public class MicrodataAnalyzer {
	...
	public void extractDataCodes(String inputFile, ArrayList<MicroData> dataList){		
		...
		//2004~2012: year(15,19), location(0,7), industry(7,13), workers(107,112), business(13,14), store(14,15)
		...
		try{
			...
			while(scan.hasNext()){	
				...
				tmpStr = microdataCode.substring(17, 21);
				if(tmpStr.contains(" ") == false){
					tmpInt = Integer.parseInt(tmpStr);
					if(tmpInt>1900 && tmpInt<2013) tmpMdata.setFoundedYear(tmpInt);
					else check[1]++;
				}else check[1]++;
				...
				tmpStr = microdataCode.substring(23, 29);
				if(tmpStr.contains(" ") == false){
					tmpMdata.setIndustryCode(tmpStr);
				}else check[3]++;
				
				//extract workers number
				tmpStr = microdataCode.substring(199, 209).trim();
				if(tmpStr.contains(" ") == false){
					tmpInt = Integer.parseInt(tmpStr.trim());
					tmpMdata.setWorkers(tmpInt);
				}else check[4]++;	

				//extract type of business entity
				tmpStr = microdataCode.substring(15, 16);
				...
				tmpStr = microdataCode.substring(16, 17);
				if(tmpStr.equals(" ") == false){
					tmpInt = Integer.parseInt(tmpStr);
	...
	public static void main(String[] args){		
		...
		String foundingYearFile;
		ArrayList<Integer> foundingYears;		
		...
		//foundingYears = new ArrayList<Integer>(); 
		//foundingYearFile =  filePath+"extracted/"+year+"_foundingYear.txt";
		//ma.extractFoundingYear(microdataFile, foundingYears);
		//ma.makeFoundingYearDataFile(foundingYearFile, foundingYears);
